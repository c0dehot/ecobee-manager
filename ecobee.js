 ...
 * 
 * Understanding ecobee login system: https://www.ecobee.com/home/developer/api/examples/ex1.shtml
 */
...
function mailSend( subject, text ){
    return new Promise( (resolve,reject) => {
        ...
        if( DEBUG ) console.log( `Sending email...` )
...
async function appRun( retryOnError=true ) {
    ...
    let settings = {}
    if( DEBUG ) console.log( `[appRun] starting` )
    try { 
    ...
    if( !settings.authCode ){
        ...
        const url = `https://api.ecobee.com/authorize?response_type=ecobeePin&client_id=${API_KEY}&scope=smartRead`
        if( DEBUG ) console.log( ` .. getting authCode, url: ${url}` )
        const response = await axios.get(url)
        if( DEBUG ) console.log( ` ... response: `, response )
        const pin = response.data.ecobeePin
        settings.authCode = response.data.code
        if( DEBUG ) console.log( ` .... pin(${pin}) authCode(${settings.authCode})` )
    ...
    if( !settings.access_token ){
        ...
        const data = `client_id=${API_KEY}&` +( settings.refresh_token ? 
            `grant_type=refresh_token&code=${settings.refresh_token}` :
            `grant_type=ecobeePin&code=${settings.authCode}` )
        if( DEBUG ) console.log( ` .. getting access_token, url: ${url} / ${data}` )
